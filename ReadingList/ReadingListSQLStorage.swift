...
class ReadingListSQLStorage: ReadingListStorage {
    ...
    fileprivate func rowToDictionary(_ row: Row) -> AnyObject {
        ...
        result["client_id"] = NSNumber(value: row.get(ItemColumns.ClientId) as Int64)
        result["client_last_modified"] = NSNumber(value: row.get(ItemColumns.ClientLastModified) as Int64)
        result["id"] = row.get(ItemColumns.Id)
        result["last_modified"] = NSNumber(value: row.get(ItemColumns.LastModified) ?? 0 as Int64)
